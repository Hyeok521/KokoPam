...
namespace Server2
{
    public partial class Server_room : Form
    {
        ...
        void h_client_OnDisconnected(TcpClient clientSocket) // client 접속 해제 핸들러
        {
            if (clientList.ContainsKey(clientSocket))
            {
                ...
                this.Invoke(new Action(delegate () { DisplayText(displayMessage); })); // 서버 쪽 메시지 창에 출력
        ...
        private void btnXml_Click(object sender, EventArgs e)
        {
            ...
            {
                using (XmlWriter wr = XmlWriter.Create(@"C:\Users\super\Desktop\chat\file.xml"))
                {
        ...
        private void room_send_Click(object sender, EventArgs e)
        {
            // 모두에게 메세지 보내기
            try
            {
                room_message.Focus();
                string message_all = this.room_message.Text;
