...
namespace Client
{
    public partial class Client : Form
    {
        ...
        private void reserve_Click(object sender, EventArgs e)
        {
            ...
            //
            reserve.reserve_send reserve = new reserve.reserve_send();
            DialogResult result = reserve.ShowDialog(this);  // 'DialogResult'를 사용하여 'reserve_send' 폼이 닫힐 때까지 'Client' 폼을 비활성화합니다.

            if (result == DialogResult.OK) // OK 버튼이 눌렸다면 예약된 메시지를 보냅니다.
            {
                // Timer를 사용하여 예약된 시간 후에 메시지를 보낼 수 있습니다.
                System.Windows.Forms.Timer timer = new System.Windows.Forms.Timer() { Interval = (int)reserve.ReservedTime.TotalMilliseconds };
                timer.Tick += (s, ee) =>
                {
                    byte[] buffer = Encoding.Unicode.GetBytes(reserve.ReservedMessage + "$");
                    stream.Write(buffer, 0, buffer.Length);
                    stream.Flush();
                    timer.Stop();
                };
        ...
        private void emoticon_Click(object sender, EventArgs e)
        {
            ...
            //
            Emoticon.ClientTextBox = this.textBoxMessage; // textBoxMessage는 Client 폼의 TextBox입니다.
            //
